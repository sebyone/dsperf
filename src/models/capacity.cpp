...
rt_t report_capacity(const model_info_t *info_, capacity_vars_t *vars_, report_item_t switch_)
{
    ...
    case _OUTS_CSV_ROW:
        ...
        printf("%.3f\t", vars_->conntime);
        printf("%.3f\t%.3f\t%.3f\n", vars_->throughput, vars_->throughput / 8, (vars_->conntime / 1000) / vars_->pktssent);
        break;
    ...
    case _OUTS_SUMMARY:
        ...
        printf("                    %.3f [pps]\n", (vars_->conntime / 1000.0) / vars_->pktssent);
